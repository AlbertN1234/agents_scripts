...
finalize_yum_repo ()
{
  if [ "$_skip_pygpgme" = 0 ]; then
    ...
    yum install -y pygpgme --disablerepo='loadimpact_server-metrics-agent'
    ...
    if [ "$?" != "0" ]; then
      ...
      sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/loadimpact_server-metrics-agent.repo
    fi
  fi

  echo "Installing yum-utils..."
  yum install -y yum-utils --disablerepo='loadimpact_server-metrics-agent'
  ...
  echo "Generating yum cache for loadimpact_server-metrics-agent..."
  yum -q makecache -y --disablerepo='*' --enablerepo='loadimpact_server-metrics-agent'

  echo "Generating yum cache for loadimpact_server-metrics-agent-source..."
  yum -q makecache -y --disablerepo='*' --enablerepo='loadimpact_server-metrics-agent-source'
}

finalize_zypper_repo ()
{
  zypper --gpg-auto-import-keys refresh loadimpact_server-metrics-agent
  zypper --gpg-auto-import-keys refresh loadimpact_server-metrics-agent-source
}
...
main ()
{
  ...
  yum_repo_config_url="https://packagecloud.io/install/repositories/loadimpact/server-metrics-agent/config_file.repo?os=${os}&dist=${dist}&source=script"

  if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
    yum_repo_path=/etc/zypp/repos.d/loadimpact_server-metrics-agent.repo
  else
    yum_repo_path=/etc/yum.repos.d/loadimpact_server-metrics-agent.repo
  fi
